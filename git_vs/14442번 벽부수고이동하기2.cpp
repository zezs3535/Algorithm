...
int dx[4] = { 1,0,-1,0 };
int dy[4] = { 0,1,0,-1 };
int visited[1001][1001][15];

struct INFO { //좌표값, 방문cnt, 벽을 몇번깼는지
	int y;
	int x;
	int cnt;
	int isBreak;
...
void bfs(vector<vector<int>>& arr, int ypos, int xpos) {
	...
	while (!q.empty()) {
		...
		for (int idx = 0; idx < 4; idx++) {
			...
			if (arr[ny][nx] == -9 && cur.isBreak == breakCnt)continue; //벽인데 이미 한번 깼으면 ㅌㅌ
			if (arr[ny][nx] == -9 && cur.isBreak != breakCnt) { //벽인데 아직 안깼으면 하나 깬다
				q.push({ ny,nx,cur.cnt + 1,cur.isBreak+1 });
